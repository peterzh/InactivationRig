...
%             obj.monitor = MonitorController(obj.monitorDevice);
            ...
            load \\zubjects.cortexlab.net\Code\Rigging\ExpDefinitions\Peter\31CoordSet;
        ...
        function scanLine(obj,period)
            if isempty(obj.thorcam.ste2pos_transform)
                error('Calibrate stereotaxic grid');
            end
            
            laserVolt = obj.laser.power2volt(0.1, 'DC');

                            
            totalTime = 30; %Total duration of laser stimulation
            
            %Coordinates defining the points of the line
            steCoords = [0, 0; %A
                         0, -4]; %B
            
            pos = obj.thorcam.ste2pos(steCoords);
            volt = obj.galvo.pos2v(pos);
            
            
            rate = obj.galvo.daqSession.Rate;
            t = 0:(1/rate):totalTime; t(1)=[];
            
            baseWave = (sawtooth(2*pi*t/period)+1)/2;

            waveX = baseWave*diff(volt(:,1)) + volt(1,1);
            waveY = baseWave*diff(volt(:,2)) + volt(1,2);
            ...
            %Turn laser on
            obj.laser.daqSession.outputSingleScan(laserVolt);
            
            %Issue galvo waveform
...
%             t = 0:(1/rate):totalTime; t(1)=[];
        ...
        function saveLog(obj)
            log = obj.log;
            log.stereotaxCalib = obj.thorcam.log_stereotaxImg;
