...
if iscell(eventObj.Data) && strcmp(eventObj.Data{2}, 'experimentInit') %Experiment started
    ...
    LGObj.filepath = [dat.expPath(expRef, 'main', 'm') '\' expRef '_galvoLog.mat'];
...
elseif isstruct(eventObj.Data) && any(strcmp({eventObj.Data.name},'events.newTrial'))
    ...
    try
        ...
        %Specify the laser waveform type (Sine or DC)
        if any(strcmp(names,'events.waveformType')) % 1 = Truncated Sine wave, 2 = DC 
            waveformType = values{strcmp(names,'events.waveformType')};
        else
            waveformType = 1;
        end
...
%             disp(['    delay_preallocLaserWaveform: ' num2str(ROW.delay_preallocLaserWaveform)]);
            ...
            fprintf(['%0.1fmW (%0.2fv) '], laserPower, laserVolt);
...
%             disp(['    delay_vidHighlight: ' num2str(ROW.delay_vidHighlight)]);
    ...
    elseif galvoType == 2 % MULTI SCAN MODE
        if waveformType==2
           error('DC waveform not compatible with galvoType=2'); 
        end
...
%             laserVolt = laserPower;
    ...
    ROW.laserDuration = laserDuration;
    ROW.waveformType = waveformType;
    ROW.tictoc = toc(allT);
